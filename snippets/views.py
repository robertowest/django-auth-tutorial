...
def edit_snippet(request, snippet_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.add_message(request, messages.SUCCESS, "Fragmentos actualizados.")
...
def new_comment(request, snippet_id):
    ...
    if form.is_valid():
        ...
        comment.save()
        messages.add_message(request, messages.SUCCESS, "Comentario publicado")
    else:
        messages.add_message(request, messages.ERROR, "Error al publicar comentario")
    return redirect('snippet_detail', snippet_id=snippet_id)
